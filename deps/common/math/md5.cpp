...
static void MD5_memcpy(POINTER, const unsigned char *, unsigned int);
...
void MD5Update(MD5_CTX *context, unsigned char *input, unsigned int inputLen)
{
  ...
  context->count[0] += (static_cast<UINT4>(inputLen) << 3);
   ...
   */
  if (inputLen >= partLen) {
    MD5_memcpy(static_cast<POINTER>(&context->buffer[index]), static_cast<POINTER>(input), partLen);
  ...
  MD5_memcpy(static_cast<POINTER>(&context->buffer[index]), static_cast<POINTER>(&input[i]), inputLen - i);
}
...
int MD5Buffer(char *buffer, unsigned int len, unsigned char digest[16])
{
  ...
  MD5Init(&context);
  MD5Update(&context, reinterpret_cast<unsigned char *>(buffer), len);
  MD5Final(digest, &context);
  return 0;
}
