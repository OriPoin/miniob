...
RC TableScanVecPhysicalOperator::open(Trx *trx)
{
  ...
  // TODO(unknown): don't need to fetch all columns from record manager
...
RC TableScanVecPhysicalOperator::next(Chunk &chunk)
{
  ...
  if (OB_SUCC(rc)) {
    ...
    if (predicates_.empty()) {
      ...
      // TODO(unknown): if all setted, it doesn't need to set one by one
      for (int i = 0; i < all_columns_.rows(); i++) {
        ...
        for (int j = 0; j < all_columns_.column_num(); j++) {
          filterd_columns_.column(j).append_one(
              const_cast<char *>(all_columns_.column(filterd_columns_.column_ids(j)).get_value(i).data()));
        }
      }
