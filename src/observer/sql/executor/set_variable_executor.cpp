...
RC SetVariableExecutor::execute(SQLStageEvent *sql_event)
{
  ...
  auto    *stmt    = static_cast<SetVariableStmt *>(sql_event->stmt());
...
RC SetVariableExecutor::var_value_to_boolean(const Value &var_value, bool &bool_value)
{
  RC rc = RC::SUCCESS;
  ...
  if (var_value.attr_type() == AttrType::BOOLEANS) {
    ...
    for (const auto &true_string : true_strings) {
      if (strcasecmp(var_value.get_string().c_str(), true_string.c_str()) == 0) {
        bool_value = true;
        return rc;
      }
    }
    ...
    for (const auto &false_string : false_strings) {
      if (strcasecmp(var_value.get_string().c_str(), false_string.c_str()) == 0) {
        bool_value = false;
        return rc;
      }
    }
...
RC SetVariableExecutor::get_execution_mode(const Value &var_value, ExecutionMode &execution_mode)
{
  RC rc = RC::SUCCESS;
