...
RC StandardAggregateHashTable::Scanner::next(Chunk &output_chunk)
{
  ...
  while (it_ != end_ && output_chunk.rows() <= output_chunk.capacity()) {
    const auto &group_by_values = it_->first;
    ...
    for (int i = 0; i < output_chunk.column_num(); i++) {
      ...
      if (col_idx >= static_cast<int>(group_by_values.size())) {
        output_chunk.column(i).append_one(const_cast<char *>(aggrs[col_idx - group_by_values.size()].data()));
      } else {
        output_chunk.column(i).append_one(const_cast<char *>(group_by_values[col_idx].data()));
      }
    }
    it_++;
  }
