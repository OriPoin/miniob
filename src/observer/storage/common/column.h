 ...
 */
// TODO(unknown): `Column` currently only support fixed-length type.
class Column
{
public:
  enum class Type
  ...
  explicit Column(const FieldMeta &meta, size_t capacity = DEFAULT_CAPACITY);
  Column(AttrType attr_type, size_t attr_len, size_t capacity = DEFAULT_CAPACITY);

  void init(const FieldMeta &meta, size_t size = DEFAULT_CAPACITY);
  void init(AttrType attr_type, size_t attr_len, size_t capacity = DEFAULT_CAPACITY);
   ...
   * @brief 获取列数据的实际大小（字节）
   */
  [[nodiscard]] int data_len() const { return count_ * attr_len_; }
  ...
  [[nodiscard]] int      attr_len() const { return attr_len_; }
  [[nodiscard]] Type     column_type() const { return column_type_; }
