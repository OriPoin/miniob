...
RC JavaThreadPoolThreadHandler::start()
{
  ...
  auto event_worker = [this] { event_loop_thread(); };
 ...
 */
static void event_callback(evutil_socket_t fd, int16_t event, void *arg)
{
  if (event & (EV_READ | EV_CLOSED)) {
    LOG_TRACE("got event. fd=%d, event=%d", fd, event);
    auto                        *ag      = static_cast<EventCallbackAg *>(arg);
